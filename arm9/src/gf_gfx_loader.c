...
u32 GfGfxLoader_LoadWholePalette(NarcId narcId, s32 memberId, u32 layer, u32 baseAddr, u32 szByte, BOOL isCompressed, enum HeapID heapID) // todo: change this to new format
{
    static void (*const load_funcs[])(void *, u32, u32) = {
        GX_LoadOBJ,
        GXS_LoadOBJ
...
void *GfGfxLoader_LoadFromNarc(NarcId narcId, s32 memberNo, BOOL isCompressed, enum HeapID heapID, BOOL allocAtEnd) {
    ...
    if (isCompressed || allocAtEnd == TRUE) {
        ...
        dest = Heap_AllocAtEnd(heapID, size);
    } else {
        u32 size = GetNarcMemberSizeByIdPair(narcId, memberNo);
        dest = Heap_Alloc(heapID, size);
    }
    if (dest != NULL) {
        ReadWholeNarcMemberByIdPair(dest, narcId, memberNo);
        if (isCompressed) {
            if (allocAtEnd == FALSE) {
                dest2 = Heap_Alloc(heapID, (*(u32 *)dest) >> 8);
            } else {
                dest2 = Heap_AllocAtEnd(heapID, (*(u32 *)dest) >> 8);
            }
            if (dest2 != NULL) {
...
void *GfGfxLoader_LoadFromNarc_GetSizeOut(NarcId narcId, s32 memberNo, BOOL isCompressed, enum HeapID heapID, BOOL allocAtEnd, u32 *size_p) {
    ...
    *size_p = GetNarcMemberSizeByIdPair(narcId, memberNo);
    if (isCompressed || allocAtEnd == TRUE) {
        dest = Heap_AllocAtEnd(heapID, *size_p);
    } else {
        dest = Heap_Alloc(heapID, *size_p);
    }
    if (dest != NULL) {
        ReadWholeNarcMemberByIdPair(dest, narcId, memberNo);
        if (isCompressed) {
            *size_p = (*(u32 *)dest) >> 8;
            if (allocAtEnd == FALSE) {
                dest2 = Heap_Alloc(heapID, *size_p);
            } else {
                dest2 = Heap_AllocAtEnd(heapID, *size_p);
            }
            if (dest2 != NULL) {
                MI_UncompressLZ8(dest, dest2);
                Heap_Free(dest);
