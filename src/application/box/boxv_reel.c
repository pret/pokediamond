...
static void trayicon_free_callback(void* wk_adrs);
...
BOOL BoxAppView_Reel_Init( REEL_VIEW_WORK** wk_ptr_adrs, BOXAPP_VIEW_WORK* vwk, const BOXAPP_VPARAM* vpara, GF_BGL_INI* bgl, CLACT_SET_PTR actsys )
{
	...
	{
		REEL_VIEW_WORK* wk = sys_AllocMemory( HEAPID_BOX_VIEW, sizeof(REEL_VIEW_WORK) );
		if( wk )
		{
			int i;
...
//	OS_TPrintf("  *** ReelView: move=%d, pos=%d\n", wk->vpara->subLCD.move, trayno);
...
static void trayicon_appear_task( TCB_PTR tcb, void* wk_adrs )
{
	...
	case 2:
		...
		{
			cwk->seq++;
			BoxAppView_IrqMemoryFreeReq(cwk, trayicon_free_callback, wk);
		}
		break;
	}
}
