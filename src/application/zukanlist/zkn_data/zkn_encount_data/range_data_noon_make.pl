...
#			@work = (@work,  $enc_localfile[71], $enc_localfile[74], $enc_localfile[77], $enc_localfile[80], $enc_localfile[83] );
			...
			#昼
			$work[2] = $enc_localfile[28];
			$work[3] = $enc_localfile[29];
			@work = &mons_name_arry_mons_no_chg(@work);
			$encmons_normal[ $count ] = join( ',', @work );
	...
	for( $count=0; $count<@MONS_ID; $count++ ){
		#ヒル
		&make_mons_range_data( $count, \@encmons_normal, \@zone_id, \@RANGE_DAN_NU, \@RANGE_FLD_NU );
	}
}
...
sub make_pokerangedata{
	...
	#ダンジョン昼分布ファイル
	&make_mons_rang_out_file_dan( \@RANGE_DAN_NU, "zkn_dan_range_noon_mons", "モンスターダンジョン昼分布ID\r\n" );

	#フィールド昼分布ファイル
	&make_mons_rang_out_file_fld( \@RANGE_FLD_NU, "zkn_fld_range_noon_mons", "モンスターフィールド昼分布ID\r\n" );
}
