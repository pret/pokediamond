...
static BOOL Poketch_A47_Init(void **wk_ptr, CONTACT_WORK* conwk, GF_BGL_INI* bgl, u32 param);
static void Poketch_A47_Main(TCB_PTR tcb, void *wk_adrs);
static void Poketch_A47_Quit(void *wk_adrs);
...
static void NitroStaticInit(void)
{
	PoketchSetAppProc(Poketch_A47_Init, Poketch_A47_Quit);
}
...
static BOOL Poketch_A47_Init(void **wk_ptr, CONTACT_WORK* conwk, GF_BGL_INI* bgl, u32 param)
{
	...
	if(wk != NULL)
	{
		...
		{
			if( TCB_Add(Poketch_A47_Main, wk, POKETCH_APP_TASKPRI) != NULL )
			{
				*wk_ptr = wk;
				return TRUE;
			}
...
static BOOL SetupWork( APPWORK *wk, CONTACT_WORK* conwk, GF_BGL_INI* bgl, u32 param )
{
	if( Poketch_A47_SetViewWork( &(wk->vwk), &(wk->vpara), bgl ) )
	{
...
static void CleanupWork( APPWORK *wk )
{
	Poketch_A47_DeleteViewWork( wk->vwk );
	sys_FreeMemoryEz( wk );
}
//------------------------------------------------------------------
/**
...
static BOOL SeqInit(APPWORK *wk)
{
	switch(wk->seq){
	case 0:
		Poketch_A47_SetViewCommand( wk->vwk, CMD_INIT );
		wk->seq++;
		break;

	case 1:
		if(Poketch_A47_WaitViewCommand( wk->vwk, CMD_INIT ))
		{
...
static BOOL SeqQuit(APPWORK *wk)
{
	switch(wk->seq){
	case 0:
		Poketch_A47_SetViewCommand( wk->vwk, CMD_QUIT );
		wk->seq++;
		break;

	case 1:
		if(Poketch_A47_WaitViewCommandAll( wk->vwk ))
		{
			return TRUE;
		}
		break;
